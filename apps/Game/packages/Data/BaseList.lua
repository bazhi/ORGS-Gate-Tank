...
function BaseList:load(db, where)
    local template = self:getTemplate()
    local result, err = template:selectQuery(db, where)
    if err then
        cc.printf(err)
        return nil
    end
...
function BaseList:save(db)
    self._Datas = self._Datas or {}
    local values = self._Datas
    for _, v in ipairs(values) do
        local _, err = v:save(db)
        if err then
            cc.printf("db save:"..err)
        end
    end
end
...
function BaseList:updateAll(values)
    local bupdate = false
    for _, v in ipairs(values) do
        bupdate = true
        self:update(v)
